...
class OCR {
  ...
  function getValues($text) {
    $origen = ["Cuenta de origen CA ","Cuenta de origen CA USD","de la Caja de Ahorro en pesos No.","de la Caja de Ahorro en dólares No.","Cuenta de origen CA UYU","de\nItaú\ncuenta","Origen CC en Dolares","Cuenta de origen CC USD"," de cuenta:"];
    ...
    $pay = ["Importe a acreditar","Importe a acrecitar","Monto","Total en ","Monto acreditado:","Debitamos","Monto total","Importe giro:","Moneda y monto",'Importe a USD","importe transferido uU$S',"importe a acreditar","importe transferido","importe transferido"];
    $bank = ["Banco","Banco"];
    $moneda =  ["Moneda"];
  ...
  function primeraPalabraEncontrada($texto, $palabras) {
    foreach ($palabras as $palabra) {
        if (strpos($texto, $palabra) !== false) {
            return $palabra;
        }
    }
    return false; // Devuelve null si no encuentra ninguna palabra
}
  ...
  function searchBank($haystack, $needles, $offset = 0) {
    ...
    foreach (["REPÚBLICA","itaú","santander","scotiabank","bbva","REPÚBLICA"] as $bank) {
    ...
    return $this->primeraPalabraEncontrada(strtolower($haystack), [strtolower("REPÚBLICA"),"itaú","santander","scotiabank","bbva"]);
  }
